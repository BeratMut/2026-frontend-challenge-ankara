...
import { getFormSubmissions } from "../services/messagesApi";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Messages Hatası:", err);
      } finally {
        setLoading(false);
      }
    };
        ...
        {submissions.length > 0 ? (
          submissions.map((submission, idx) => (
            <div
              ...
              className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow p-6 border-l-4 border-green-500"
            >
              {/* Card Header */}
